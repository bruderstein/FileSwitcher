...
void SwitchDialog::doDialog(SimpleFileContainer files)
{
	_ctrlTabSearch = false;
	...
	searchFiles(searchString, files);
	...
	if (_options->defaultSortOrder != ALWAYSREMEMBER
		&& _options->resetSortOrder
	){
		_options->activeReversedSortOrder = _options->reversedSortOrder;
		_listView.sortSimpleFileItems(_options->defaultSortOrder);
	}
	else
	{
		_listView.sortSimpleFileItems(_options->activeSortOrder);
	}
...
{
	::SetWindowPos(_hSelf, _hParent, _dialogX, _dialogY, _dialogWidth, _dialogHeight, SWP_SHOWWINDOW);
}

void SwitchDialog::setupColumnWidths(SimpleFileContainer &files)
{
	if (_options->autoSizeColumns)
	{
		...
		for(SimpleFileContainer::iterator itr = files.begin(); itr != files.end(); itr++)
		{
			tempWidth = ListView_GetStringWidth(_hListView, itr->second->fileName.c_str());
			if (tempWidth > filenameColumnMaxWidth)
				filenameColumnMaxWidth = tempWidth;

			tempWidth = ListView_GetStringWidth(_hListView, itr->second->filePath.c_str());
	...
	case WM_KEYDOWN :
		{
			...
			default:
				...
				if (_options->emulateCtrlTab
					&& !_overrideCtrlTab
					&& _options->revertSortWhenTabbing)
				{
			...
			case IDC_FILEEDIT:
				{
					if (HIWORD(wParam) == EN_CHANGE)
					{
						...
						if(_options->hasConfiguredContext
							&& !_ctrlTabSearch)
						{
							searchFiles(searchString, simpleFiles);
						}
						else
						{
			...
			default:
				{
					...
					case IDOK :
						{
							TCHAR columnInfo[50];
							getColumnWidths(columnInfo, 50);
							_tcscpy_s(_options->columnWidths, COLUMNWIDTH_LENGTH, columnInfo);
...
void SwitchDialog::OpenSimpleFile()
{
	SimpleFileInfo* simpleFile = _listView.getCurrentSimpleFile();
	...
	if (simpleFile != NULL)
	{
		...
		_tcscpy_s(fullFileName, MAX_PATH, tstring().append(simpleFile->filePath).append(simpleFile->fileName).c_str());
		::SendMessage(_nppData._nppHandle, NPPM_DOOPEN, 0, (LPARAM)fullFileName);
	}
}
...
bool tstrReplace(tstring& str, const tstring& from, const tstring& to) {
...
tstring SwitchDialog::initSearchString(tstring str)
{
	tstring result = str;

	//escape regex chars
	tstrReplace(result, _T("\\"), _T("\\\\"));
	tstrReplace(result, _T("^"), _T("\\^"));
	tstrReplace(result, _T("$"), _T("\\$"));
	tstrReplace(result, _T("."), _T("\\."));
	tstrReplace(result, _T("|"), _T("\\|"));
	...
	if (_options->searchFlags & SEARCHFLAG_STARTONLY)
		...
		result = tstring(_T(".*")).append(result.append(_T(".*")));
...
{
	//this is only here in case the plugin crashes due to the searching taking too long (dll entry point gets re-ran).
	//  The long term solution would be to have a separate thread run, but I chose not to spend the time.
	if(files.size() == 0)
	{
	...
	SimpleFileContainer::iterator itr = files.begin();
	bool include;
	...
	while(itr != files.end() && (count < _options->maxDisplayFiles))
	{
		...
		if (_options->searchFlags & SEARCHFLAG_STARTONLY)
		{
			...
			{
				base = itr->second->filePath;
				include = std::regex_match(base, rx);
			}
			if (!include && (_options->searchFlags & SEARCHFLAG_INCLUDEFILENAME))
			{
				base = itr->second->fileName;
				include = std::regex_match(base, rx);
			}
		}
		else
		{
			...
			{
				base = tstring(_T("")).append(itr->second->filePath).append(itr->second->fileName);
			}
		...
		if (include)
		{
			...
			addListEntry((SimpleFileInfo *)itr->second, selected);
...
void SwitchDialog::searchFiles(TCHAR* search, int selectedEditFileView, int selectedEditFileIndex)
{
	...
	while(iter != _editFiles.end())
	{
		...
		if (_options->searchFlags & SEARCHFLAG_STARTONLY)
		{
			if (_options->searchFlags & SEARCHFLAG_INCLUDEPATH)
			{
				base = iter->second->getPath();
				include = std::regex_match(base, rx);
			}
			if (!include && (_options->searchFlags & SEARCHFLAG_INCLUDEFILENAME))
			{
				base = iter->second->getFilename();
				include = std::regex_match(base, rx);
			}

			if (!include && (_options->searchFlags & SEARCHFLAG_INCLUDEINDEX))
			{
				include = (_ttoi(search) - 1 == iter->second->getIndex())
					|| (search[0] == '\0');
			}

			if (!include && (_options->searchFlags & SEARCHFLAG_INCLUDEVIEW))
			{
				include = (_ttoi(search) - 1 == iter->second->getView())
					|| (search[0] == '\0');
			}
		}
		else
		{
			if ((_options->searchFlags & SEARCHFLAG_INCLUDEPATH)
				&& (_options->searchFlags & SEARCHFLAG_INCLUDEFILENAME))
			{
				base = iter->second->getFullFilename();
			}
			else if (_options->searchFlags & SEARCHFLAG_INCLUDEPATH)
			{
				base = iter->second->getPath();
...
void SwitchDialog::addListEntry(SimpleFileInfo *file, bool selected)
{
...
int SwitchDialog::getNumColumns()
{
	return _listView.getColumnSize();
}